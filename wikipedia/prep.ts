...
const text = await Deno.readTextFile("working_on/formatdetect/wikipedia/train_data.csv");
...
for (const key of map.keys()) {
    Deno.writeTextFile(`working_on/formatdetect/wikipedia/data/${key}.html`, map.get(key)!.join("\n")).then(() => console.log("Did", key));
}